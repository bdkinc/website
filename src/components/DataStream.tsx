...
export default function DataStream({
  ...
  useEffect(() => {
    ...
    if (variant === 'matrix') {
      ...
      const chars =
        '01アイウエオカキクケコサシスセソタチツテトナニヌネノハヒフヘホマミムネモヤユヨラリルレロワヲン';
      ...
      lines.forEach((line, i) => {
        const stream = document.createElement('div');
        stream.className =
          'absolute whitespace-nowrap text-primary animate-scroll-left font-mono text-xs';
  ...
  return (
    <div
      ref={containerRef}
      className={`pointer-events-none absolute inset-0 overflow-hidden ${variant === 'horizontal' ? 'opacity-10' : ''} ${className}`}
      aria-hidden="true"
    />
  );
}
